...
AI生成的一个较高效的B站视频下载工具，支持单个视频和UP主视频批量下载。

思路：指导AI利用selenium模拟用户打开浏览器获取up的全部视频的BV号（避开bilibili的反爬策略），然后根据这些BV号将该up的所有视频下载。
...
1. 本工具仅用于学习研究，请勿用于商业用途
2. 用selenium打开浏览器的时候，会点击一下b站的登录页面，这个时候最好登录一下，因为会存在如果不登录用户则不能看up的完整的空间的情况，影响后续功能。
